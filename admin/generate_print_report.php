...
$total_completed_bookings = ($revenue_stats['total_bookings'] ?: 0) - ($revenue_stats['cancelled_bookings'] ?: 0) - ($revenue_stats['no_show_bookings'] ?: 0);
$cancellation_rate = $revenue_stats['total_bookings'] > 0 ?
    (($revenue_stats['cancelled_bookings'] ?: 0) / $revenue_stats['total_bookings']) * 100 : 0;
$no_show_rate = $revenue_stats['total_bookings'] > 0 ?
    (($revenue_stats['no_show_bookings'] ?: 0) / $revenue_stats['total_bookings']) * 100 : 0;
...
if ($therapist_result && $therapist_result->num_rows > 0) {
...
function exportToExcel($conn, $start_date, $end_date, $report_type)
{
    ...
    $revenue_result = $conn->query($revenue_sql);
    $revenue_stats = $revenue_result ? $revenue_result->fetch_assoc() : [];
    ...
    echo '<html xmlns:o="urn:schemas-microsoft-com:office:office"
          ...
          <style>
            ...
            .header { background-color: #34495e; color: white; font-size: 16px; font-weight: bold; }
            .section-header { background-color: #7f8c8d; color: white; font-weight: bold; }
          </style>
          </head><body>';
    ...
    // หัวข้อรายงาน
    echo '<tr><td colspan="6" class="header">รายงานสถิติธุรกิจนวดไทย</td></tr>';
    ...
    // สถิติหลัก
    echo '<tr><td colspan="6" class="section-header">สถิติหลัก (KPIs)</td></tr>';
    echo '<tr>
            <th>รายได้รวม</th>
            <th>การจองทั้งหมด</th>
            <th>ลูกค้าทั้งหมด</th>
    ...
    // สถิติบริการ
    echo '<tr><td colspan="6" class="section-header">สถิติบริการ</td></tr>';
    echo '<tr>
            <th>บริการ</th>
            <th>จำนวนการจอง</th>
            <th>รายได้รวม</th>
    ...
    if ($service_result && $service_result->num_rows > 0) {
        ...
        echo '<tr><td colspan="6" style="text-align: center;">ไม่มีข้อมูลบริการ</td></tr>';
    }
    echo '<tr><td colspan="6">&nbsp;</td></tr>';

    // สถิติพนักงาน
    echo '<tr><td colspan="7" class="section-header">สถิติพนักงานนวด</td></tr>';
    echo '<tr>
            <th>ชื่อพนักงาน</th>
            <th>จำนวนการจอง</th>
            <th>รายได้รวม</th>
    ...
    if ($therapist_result && $therapist_result->num_rows > 0) {
        ...
        echo '<tr><td colspan="7" style="text-align: center;">ไม่มีข้อมูลพนักงาน</td></tr>';
    }
    echo '<tr><td colspan="7">&nbsp;</td></tr>';

    // สถิติลูกค้า
    echo '<tr><td colspan="6" class="section-header">ลูกค้ายอดนิยม (Top 10)</td></tr>';
    echo '<tr>
            <th>ชื่อลูกค้า</th>
            <th>อีเมล</th>
            <th>เบอร์โทร</th>
    ...
    if ($customer_result && $customer_result->num_rows > 0) {
        ...
        echo '<tr><td colspan="6" style="text-align: center;">ไม่มีข้อมูลลูกค้า</td></tr>';
    }
...
<body>
    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="h3 mb-1"><i class="bi bi-graph-up"></i> ระบบรายงานและสถิติเชิงวิเคราะห์</h1>
                <p class="text-muted mb-0">ระบบวิเคราะห์ข้อมูลธุรกิจนวดเพื่อการตัดสินใจทางธุรกิจ</p>
            </div>
            <div>
                <button class="btn btn-success" onclick="exportToExcel()">
                    <i class="bi bi-file-earmark-excel"></i> ส่งออกรายงาน Excel
                </button>
            </div>
        </div>
        ...
        <div class="card mb-4">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bi bi-funnel"></i> ตัวกรองข้อมูล</h5>
            </div>
            <div class="card-body">
                <form method="GET" class="row g-3">
                    <div class="col-md-3">
                        <label class="form-label fw-bold">ช่วงวันที่เริ่มต้น</label>
                        <input type="date" name="start_date" class="form-control"
                            value="<?= htmlspecialchars($start_date) ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold">ช่วงวันที่สิ้นสุด</label>
                        <input type="date" name="end_date" class="form-control"
                            value="<?= htmlspecialchars($end_date) ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold">ประเภทรายงาน</label>
                        <select name="report_type" class="form-select">
                            <option value="overview" <?= $report_type == 'overview' ? 'selected' : '' ?>>
                                ภาพรวมธุรกิจ</option>
                            <option value="services" <?= $report_type == 'services' ? 'selected' : '' ?>>
                                วิเคราะห์บริการ</option>
                            <option value="therapists" <?= $report_type == 'therapists' ? 'selected' : '' ?>>
                                วิเคราะห์พนักงาน</option>
                            <option value="customers" <?= $report_type == 'customers' ? 'selected' : '' ?>>
                                วิเคราะห์ลูกค้า</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <div class="d-grid">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-bar-chart"></i> สร้างรายงาน
                            </button>
                        </div>
                    </div>
                </form>
                <div class="mt-3">
                    <span class="badge bg-info">ช่วงเวลาที่วิเคราะห์:
                        <?= date('d/m/Y', strtotime($start_date)) ?> -
                        <?= date('d/m/Y', strtotime($end_date)) ?></span>
                    <span class="badge bg-secondary ms-2">จำนวนวัน:
                        <?= round((strtotime($end_date) - strtotime($start_date)) / (60 * 60 * 24)) + 1 ?>
                        วัน</span>
                </div>
            </div>
        </div>
            ...
            <div class="col-12 col-sm-6 col-lg-3">
                <div class="card text-white bg-primary h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h2 class="card-title mb-1">
                                    ฿<?= number_format($revenue_stats['total_revenue'] ?: 0, 2) ?></h2>
                                <small>รายได้สุทธิหลังหักค่าบริการ</small>
                            </div>
                        ...
                        <div class="mt-2">
                            <span class="badge bg-light text-dark kpi-badge">รายได้เฉลี่ยต่อวัน:
                                ฿<?= number_format(($revenue_stats['total_revenue'] ?: 0) / max(1, round((strtotime($end_date) - strtotime($start_date)) / (60 * 60 * 24)) + 1), 2) ?></span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-sm-6 col-lg-2">
                <div class="card text-white bg-success h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="card-subtitle mb-2 text-white-50">การจองทั้งหมด</h6>
                                <h2 class="card-title mb-1">
                                    <?= number_format($revenue_stats['total_bookings'] ?: 0) ?></h2>
                                <small>จำนวนการจองทั้งหมด</small>
                            </div>
                            <div class="fs-1 opacity-50">
                        ...
                        <div class="mt-2">
                            <span class="badge bg-light text-dark kpi-badge">สำเร็จ:
                                <?= number_format($total_completed_bookings) ?></span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-sm-6 col-lg-2">
                <div class="card text-white bg-info h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="card-subtitle mb-2 text-white-50">ลูกค้าทั้งหมด</h6>
                                <h2 class="card-title mb-1">
                                    <?= number_format($revenue_stats['total_customers'] ?: 0) ?></h2>
                                <small>ลูกค้าไม่ซ้ำ</small>
                            </div>
                            <div class="fs-1 opacity-50">
                        ...
                        <div class="mt-2">
                            <span class="badge bg-light text-dark kpi-badge">ลูกค้าเก่า:
                                <?= number_format($revenue_stats['total_customers'] ?: 0) ?></span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-sm-6 col-lg-2">
                <div class="card text-white bg-warning h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="card-subtitle mb-2 text-white-50">มูลค่าเฉลี่ย</h6>
                                <h2 class="card-title mb-1">
                                    ฿<?= number_format($revenue_stats['avg_revenue_per_booking'] ?: 0, 2) ?>
                                </h2>
                                <small>ต่อการจอง</small>
                            </div>
                            <div class="fs-1 opacity-50">
                        ...
                        <div class="mt-2">
                            <span class="badge bg-light text-dark kpi-badge">ARPU:
                                ฿<?= number_format(($revenue_stats['total_revenue'] ?: 0) / max(1, $revenue_stats['total_customers']), 2) ?></span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-sm-6 col-lg-3">
                <div class="card text-white bg-secondary h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="card-subtitle mb-2 text-white-50">อัตราการยกเลิก</h6>
                                <h2 class="card-title mb-1"><?= number_format($cancellation_rate, 1) ?>%</h2>
                                <small>และไม่มาใช้บริการ</small>
                            </div>
                            <div class="fs-1 opacity-50">
                        ...
                        <div class="mt-2">
                            <span class="badge bg-light text-dark kpi-badge">ยกเลิก:
                                <?= number_format($revenue_stats['cancelled_bookings']) ?> | ไม่มา:
                                <?= number_format($revenue_stats['no_show_bookings']) ?></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card mb-4">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bi bi-graph-up-arrow"></i> การวิเคราะห์แนวโน้มรายเดือน</h5>
            </div>
            <div class="card-body">
        ...
        <ul class="nav nav-tabs mb-4" id="reportTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $report_type == 'overview' ? 'active' : '' ?>" id="overview-tab"
                    ...
                    <i class="bi bi-house"></i> ภาพรวมธุรกิจ
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $report_type == 'services' ? 'active' : '' ?>" id="services-tab"
                    ...
                    <i class="bi bi-spa"></i> วิเคราะห์บริการ
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $report_type == 'therapists' ? 'active' : '' ?>" id="therapists-tab"
                    ...
                    <i class="bi bi-person-badge"></i> วิเคราะห์พนักงาน
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $report_type == 'customers' ? 'active' : '' ?>" id="customers-tab"
                    ...
                    <i class="bi bi-people"></i> วิเคราะห์ลูกค้า
                </button>
            </li>
        </ul>
        ...
        <div class="tab-content" id="reportTabsContent">
            ...
            <div class="tab-pane fade <?= $report_type == 'overview' ? 'show active' : '' ?>" id="overview"
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="card">
                            <div class="card-header bg-primary text-white">
                                <h5 class="mb-0"><i class="bi bi-graph-up"></i> รายได้และการจองรายวัน</h5>
                            </div>
                            <div class="card-body">
                                <div class="chart-container">
                    ...
                    <div class="col-lg-4">
                        <div class="card">
                            <div class="card-header bg-primary text-white">
                                <h5 class="mb-0"><i class="bi bi-credit-card"></i> วิธีการชำระเงิน</h5>
                            </div>
                            <div class="card-body">
                                <div class="chart-container">
            ...
            <div class="tab-pane fade <?= $report_type == 'services' ? 'show active' : '' ?>" id="services"
                ...
                <div class="card">
                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="bi bi-spa"></i> สถิติบริการทั้งหมด</h5>
                            ...
                            <table class="table table-hover table-striped align-middle">
                                        ...
                                        <tbody>
                                            <?php if (!empty($service_stats)): ?>
                                                <?php $total_service_revenue = array_sum(array_column($service_stats, 'total_revenue')); ?>
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="8" class="text-center text-muted py-4">
                                                        ...
                                                        <p>ไม่มีข้อมูลบริการในระยะเวลาที่เลือก</p>
                                                    </td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
            ...
            <div class="tab-pane fade <?= $report_type == 'therapists' ? 'show active' : '' ?>" id="therapists"
                ...
                <div class="card">
                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="bi bi-person-badge"></i> สถิติพนักงานนวด</h5>
                                        ...
                                        <tbody>
                                            <?php if (!empty($therapist_stats)): ?>
                                                <?php foreach ($therapist_stats as $index => $therapist): ?>
                                                    <tr>
                                                        <td><?= $index + 1 ?></td>
                                                        <td><strong><?= htmlspecialchars($therapist['full_name']) ?></strong>
                                                        </td>
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="8" class="text-center text-muted py-4">
                                                        ...
                                                        <p>ไม่มีข้อมูลพนักงานในระยะเวลาที่เลือก</p>
                                                    </td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
            ...
            <div class="tab-pane fade <?= $report_type == 'customers' ? 'show active' : '' ?>" id="customers"
                ...
                <div class="card">
                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="bi bi-people"></i> ลูกค้ายอดนิยม (Top 20)</h5>
                                        ...
                                        <tbody>
                                            <?php if (!empty($customer_stats)): ?>
                                                <?php foreach ($customer_stats as $index => $customer): ?>
                                                    <tr>
                                                        <td><?= $index + 1 ?></td>
                                                        <td><strong><?= htmlspecialchars($customer['full_name']) ?></strong>
                                                        </td>
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="9" class="text-center text-muted py-4">
                                                        ...
                                                        <p>ไม่มีข้อมูลลูกค้าในระยะเวลาที่เลือก</p>
                                                    </td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            const url = `generate_print_report.php?start_date=${startDate}&end_date=${endDate}&report_type=${reportType}&export=excel`;
